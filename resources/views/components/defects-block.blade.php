    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            @foreach($defects as $defect)
                ...
                @endphp
                @if(!$isActive && !empty($defect->resolved_url))
                    <a href="{{ $defect->resolved_url }}" 
                ...
                @else
                    <div class="flex flex-col h-full border border-gray-200 rounded-2xl p-6 group shadow-sm {{ $isActive ? 'bg-blue-50 border-l-4 border-[#0678A8]' : 'bg-white' }}">
                @endif
                    ...
                    <h3 class="text-lg font-bold text-gray-900 mb-2 {{ $isActive ? 'text-blue-700' : 'group-hover:text-blue-700 transition-colors' }}">{{ $defect->name }}</h3>
